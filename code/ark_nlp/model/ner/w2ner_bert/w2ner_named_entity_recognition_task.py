...
from ark_nlp.factory.metric import jaccard_score
...
class W2NERTask(TokenClassificationTask):
    ...
    Args:
        ...
        fgm_attack (:obj:`bool` or :obj:`None`, optional, defaults to None): 是否进行FGM对抗训练
    ...
    def _on_evaluate_step_end(self, inputs, outputs, **kwargs):
        ...
        with torch.no_grad():
            ...
            entity_text, length, label_ids = inputs['entity_text'], inputs['input_lengths'], inputs["label_ids"]
    ...
    def _on_evaluate_epoch_end(
        ...
        **kwargs
        ...
        if not hasattr(self, "best_metric"):
            self.best_metric = eval_info['recall']
        else:
            if self.best_metric <= eval_info['recall']:
                self.best_metric = eval_info['recall']

                if self.save_path and isinstance(self.save_path, str):
                    print(f"save best model to {self.save_path}_best.")
                    torch.save(self.module, f"{self.save_path}_best.pkl")
        ...
        if is_evaluate_print:
            ...
            print(f"current best metric: {self.best_metric}")

    def _on_train_end(self, **kwargs):
        if self.save_path and isinstance(self.save_path, str):
            print(f"save finetuned model to {self.save_path}.")
            torch.save(self.module, f"{self.save_path}.pkl")
            # self.module.save_pretrained(self.save_path)
